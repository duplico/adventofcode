...
def main():
     all_vals = [sf_tokenize(line.strip()) for line in fileinput.input()]

     magnitude_max = 0
     for v1 in all_vals:
          for v2 in all_vals:
               res = sf_add(v1, v2)
               # print(f"trying {sf_to_list(v1)} + {sf_to_list(v2)}\n = {sf_to_list(res)}")
               magnitude = sf_magnitude(sf_to_list(sf_add(v1, v2)))
