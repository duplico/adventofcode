...
fn main() {
    ...
    let mut cube = vec![vec![vec![0; x_sz]; y_sz]; z_sz];

    for y in 0..input_lines.len() {
        let chars: Vec<char> = input_lines[y].chars().collect();
        for x in 0..chars.len() {
            cube[ROUNDS][ROUNDS+y][ROUNDS+x] = if chars[x] == '#' { 1 } else { 0 }
        }
    }
    ...
    for _round in 0..6 {
        ...
        let mut cube_neighbors = vec![vec![vec![0; x_sz]; y_sz]; z_sz];
        for x in 0..x_sz {
            for y in 0..y_sz {
                for z in 0..z_sz {
                    if cube[z][y][x] == 1 {
                        ...
                        for x_offset in (x-1)..(x+2) {
                            for y_offset in (y-1)..(y+2) {
                                for z_offset in (z-1)..(z+2) {
                                    if x_offset == x && y_offset == y && z_offset == z {
                                        continue;
                                    }
                                    cube_neighbors[z_offset][y_offset][x_offset] += 1;
                                }
                            }
                        }
                    }
            ...
            for y in 0..y_sz {
                for z in 0..z_sz {
                    if cube[z][y][x] == 1 {
                        ...
                        // else -> inactive
                        if cube_neighbors[z][y][x] < 2 || cube_neighbors[z][y][x] > 3 {
                            cube[z][y][x] = 0;
                            active_curr -= 1;
                        }
                    } else {
    ...
    // println!("{:?}", cube_neighbors[ROUNDS]);
    println!("{:?}, {}", cube[ROUNDS], active_curr);
}
